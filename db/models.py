"""
ORM Models — SQLAlchemy 2.0 (mapped_column / DeclarativeBase).

Tables:
    products          — unique products catalog
    price_history     — each scraped price (append-only, never deleted)
    monitored_urls    — URLs that the scheduler should check
    price_alerts      — alerts generated by the engine (Day 5)

Relationships:
    Product  1 ──< PriceHistory
    Product  1 ──< PriceAlert
    MonitoredUrl 1 ── 1 Product (optional, linked after first scrape)
"""
...
class Product(Base):
    ...
    A product is identified by (url, store).
    The current price can always be obtained via the relationship
    price_history ordered by scraped_at DESC.
    """
    ...
    # Current price snapshot (denormalized for fast queries)
    ...
    # Alert threshold (null = uses global default from settings)
...
class PriceHistory(Base):
    ...
    Every time the spider scrapes a product, a record is inserted here.
    An existing record is NEVER updated or deleted.
    This is the source of truth for the change detection engine.
    """
...
class MonitoredUrl(Base):
    """
    URLs that the scheduler should review periodically.

    When a URL is added here, the scheduler scrapes it
    according to the configured interval and links the result to a Product.
    """
    ...
    # Link to the product (filled after the first scrape)
...
class PriceAlert(Base):
    """
    Price change alerts generated by the engine (Day 5).

    Every time a relevant change is detected, a record is created here.
    The notifier (Day 6) reads unnotified alerts and sends the message.
    """
    ...
    # Values: "price_drop", "price_rise", "back_in_stock",
